...
public class SearchApp2 extends PApplet {

    GDropList list;
    GTextArea txtResults;
    String items[] = {"Beverages", "Condiments", "Produce", "Meat/Poultry", "Seafood", "Dairy Products", "Grains/Cereals"};
    ...
    public void setup() {
        size(640, 480, JAVA2D);
        list = new GDropList(this, 10, 10, 100, 200, items.length);
        list.setItems(items, 0);
        txtResults = new GTextArea(this, 130, 10, 300, 300, 1);
        txtResults.setText("  Price      Product");
        txtResults.appendText("\n======   =======");
        loadData();
    }

    public void handleDropListEvents(GDropList list, GEvent event) {
        int cat = list.getSelectedIndex(); //gives back item number 0 to 7
        System.out.println(cat);
        String itemList[] = search(items[cat]);
        
        for (String currentItem: itemList){
            txtResults.appendText(currentItem+"\n");
        }
    ...
    public String[] search(String cat){
        String item[] = new String[getNumItems(cat)];
        int count = 0;
        for (int x = 0; x < category.length; x++){
            
            if (cat.equals(category[x])){
                item[count]=product[x]+"        " + price[x];
                count ++;
            }
        }
        return item;
    ...
    public int getNumItems(String cat){
        int count = 0;
